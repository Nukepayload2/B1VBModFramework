...
Namespace Logging
    ...
    Partial Public Class FileLogTraceListener
        ...
        Private ReadOnly _day As Date = Date.Today
        ...
        Private _firstDayOfWeek As Date = GetFirstDayOfWeek(Date.Today)
            ...
            Get
                ...
                Select Case LogFileCreationSchedule
                    Case LogFileCreationScheduleOption.Daily
                        fileName += $"-{Date.Today.ToString(DATE_FORMAT, CultureInfo.InvariantCulture)}"
                    Case LogFileCreationScheduleOption.Weekly
                        ' Get first day of week
                        _firstDayOfWeek = Date.Now.AddDays(-Date.Now.DayOfWeek)
        ...
        Public Property BaseFileName() As String
            ...
            Set(value As String)
                If String.IsNullOrEmpty(value) Then
                    Throw New ArgumentNullException(NameOf(value), "BaseFileName cannot be Nothing or an empty String.")
                End If
        ...
        Public Property MaxFileSize() As Long
            ...
            Set(value As Long)
                ...
                If value < MIN_FILE_SIZE Then
                    Throw New ArgumentNullException(NameOf(value), "The value of MaxFileSize must be greater than or equal to 1000.")
                End If
                _maxFileSize = value
        ...
        Public Property ReserveDiskSpace() As Long
            ...
            Set(value As Long)
                DemandWritePermission()
                If value < 0 Then
                    Throw New ArgumentNullException(NameOf(value), "The value of ReserveDiskSpace must be a positive number.")
                End If
        ...
        Private Function GetFreeDiskSpace() As Long
            ...
            Throw New Win32Exception("Cannot determine the amount of available disk space.")
        End Function
        ...
        Private Function GetStream() As ReferencedStream
            ...
            Throw New InvalidOperationException($"Unable to obtain a stream for the log.  Potential file names based on {baseStreamName} are already in use.")
        End Function
        ...
        Private Function ResourcesAvailable(newEntrySize As Long) As Boolean
            ...
            If ListenerStream.FileSize + newEntrySize > MaxFileSize Then
                If DiskSpaceExhaustedBehavior = DiskSpaceExhaustedOption.ThrowException Then
                    Throw New InvalidOperationException("Unable to write to log file because writing to it would cause it to exceed the MaxFileSize value.")
                End If
                Return False
            End If
            ...
            If GetFreeDiskSpace() - newEntrySize < ReserveDiskSpace Then
                If DiskSpaceExhaustedBehavior = DiskSpaceExhaustedOption.ThrowException Then
                    Throw New InvalidOperationException("Unable to write to log file because writing to it would reduce free disk space below ReservedSpace value.")
                End If
                Return False
            End If
        ...
        Private Function WeekChanged() As Boolean
            Return _firstDayOfWeek.Date <> GetFirstDayOfWeek(Date.Today)
        End Function
